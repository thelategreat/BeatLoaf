...
//mark it as voted for this user
//redraw this item
}
...
if ($_GET['dates'] != ''){
	...
	while ($row = mysql_fetch_array($result)){
		...
		echo "</span>";
		echo'<span class="vote"><a href="javascript:sendRequest(\'vote.php?id='.$row['name'].'\',handlevote)"><img src="images/votetop.gif"></a></span>';
		//Figure out rating
		if ($row['rating']==''){
		$row['rating'] = 0;
		}
		if ($row['rating'] <= 0){
			...
			$voteclass='votegreen';
			$voteheight = ($row['rating']/10)*25;
			$votetop = 25-$voteheight;
			$voteback='votetan';
		}
		else if ($row['rating'] <= 100){
			$voteclass='voteyellow';
			$voteheight = ($row['rating']/100)*25;
			$votetop = 25-$voteheight;
			$voteback='votegreen';
		}
		else if ($row['rating'] <= 1000){
			$voteclass='votered';
			$voteheight = ($row['rating']/1000)*25;
			$votetop = 25-$voteheight;
			$voteback='voteyellow';
		}
		
		echo '<span class="'.$voteclass.'" style="height:'.$voteheight.'px;top:'.$votetop.'px;"></span>';
		echo '<span class="'.$voteback.'" style="height:25px;top:0px;"></span>';
		echo "</div>\n";
		//echo count($songs);
