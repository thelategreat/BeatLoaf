...
def get_songs(name):
    ...
    try:
        artisturl='http://ws.spotify.com/search/1/track.json?q='+name.lower()+'&page=1'
    ...
    for song in songlist['tracks']:
        try:
            ...
            print artistName
            print urllib.quote(artistName.lower()).replace("the","").strip()
            print name.lower().replace("the","").strip()
            if urllib.quote(artistName.lower()).replace("the","").strip() == name.lower().replace("the","").strip():
                songs=songs+[{"title":song['name'],"length":song['length'],'link':song['href']}]
                print "*****Success*****"
...
def check_artistDB():
    ...
    for name in eventdict.keys():
        ...
        try:
            sql="Select idartists,asongs,refresh_date from spot_artists where idartists = '"+name+"';"
            cursor.execute (sql)
        ...
        if len(rows)>=1:
            ...
            #no, getsongs from tinysong and update
                ...
                #update
                sql="UPDATE spot_artists SET asongs ='"+json.dumps(songs)+"', refresh_date='"+str(time.time())+"'WHERE idartists ='"+name+"';"
                cursor.execute (sql)
        else:
            #no, create artist and get_songs from tinysong
                ...
                songs=get_songs(name)
                eventdict[keyname]['songID']=songs
                sql="INSERT into bnm.spot_artists(idartists, asongs, refresh_date) VALUES('"+name+"','"+json.dumps(songs)+"','"+str(time.time())+"')"
                cursor.execute (sql)
    cursor.close ()
    conn.commit ()
    conn.close ()
...
conn = MySQLdb.connect (host = "beatloaf.net",
                        ...
                        db = "beatloaf_BNM")
cursor = conn.cursor ()
sql="DELETE FROM `beatloaf_BNM`.`spot_artists` where asongs like '[]' ;"
cursor.execute (sql)
cursor.close ()
conn.commit ()
conn.close ()
